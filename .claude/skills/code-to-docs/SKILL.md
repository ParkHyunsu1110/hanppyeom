...
1. `docs/project-map.md` — 폴더/모듈 트리와 책임 (`lib/` 구조 기준)
2. `docs/build-and-test.md` — 빌드/테스트/린트/실행 명령 (`pubspec.yaml`, `firebase.json` 등 SSOT 참조)
3. `docs/architecture.md` — 데이터 흐름, 의존 방향, 외부 시스템(Firebase) 연결
4. `docs/modules/<name>.md` — 큰 모듈별 책임·공개 인터페이스·확장 포인트
5. `docs/api/<area>.md` — 외부에 노출되는 API 개요
...
1. 대상 코드 트리를 스캔한다.
   - 패키지 매니저 파일(`pubspec.yaml`)
   - 진입점 파일(`lib/main.dart`)
   - 폴더 구조와 명명 규칙
2. 모듈별로 다음을 추출한다.
   - 책임 (코드의 공개 함수/클래스/위젯이 어떤 일을 하는가)
   - 의존 (import, 외부 서비스 호출)
   - 공개 인터페이스 (export, 위젯, 라우트)
   - 데이터 모델 (Firestore 컬렉션/문서, 모델 클래스)
...
- 외부 SSOT(스펙 파일, schema, Firestore 규칙)가 있으면 본문 복제하지 말고 경로 참조.
